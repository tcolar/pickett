...
var (
	...
	runKeepEtcd = run.Flag("etcd.keep", "Keep etcd state for this topo before run (typically cleared).").Short('e').Bool()
...
func wrappedMain() int {
	...
	switch action {
	case "run":
		returnCode, err = pickett.CmdRun(*runTopo, *runVol, *runKeepEtcd, config)
	case "build":
